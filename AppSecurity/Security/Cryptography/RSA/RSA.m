...
+(NSString *)encryptRandomKey:(NSString *)randomKey withPubKey:(NSString *)publicKey{

    NSData *messageData = [randomKey dataUsingEncoding:NSUTF8StringEncoding];
    NSData *encryptedData = [RSA encryptData:messageData publicKey:publicKey];
    NSString *encryptedText = [AES hexStringFromData:encryptedData];
...
+(id)decryptMessage:(NSString *)message withPrivateKey:(NSString *)privateKey{
    
    NSError *error;
    NSData *messageData = [AES hexDataFromString:message];
    NSData *decryptedData = [RSA decryptData:messageData privateKey:privateKey];
    
    id messageDictionary = [NSJSONSerialization JSONObjectWithData:decryptedData options:NSJSONReadingMutableContainers error:&error];
